...
{
    Iterator *dfs_it =  new BfsIterator(D3);
    ASSERT_NO_THROW(dfs_it -> first());
    delete dfs_it;
}

TEST_F(BfsIteratorTest, EmptyFolderBfsIteratorNextShouldThrowException)
{
    Iterator *dfs_it =  new BfsIterator(D3);
    ASSERT_NO_THROW(dfs_it -> first());
...
TEST_F(BfsIteratorTest, EmptyFolderBfsIteratorCurrentItemShouldThrowException)
{
    Iterator *dfs_it =  new BfsIterator(D3);
    ASSERT_NO_THROW(dfs_it -> first());
    ASSERT_ANY_THROW(dfs_it -> next());
