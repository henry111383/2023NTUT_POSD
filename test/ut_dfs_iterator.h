...
TEST_F(DfsIteratorTest, EmptyFolderDfsIteratorIsDoneShouldbeCorrect)
{
    Iterator *dfs_it =  new DfsIterator(D3);
    ASSERT_NO_THROW(dfs_it -> first());
    ASSERT_NO_THROW(dfs_it -> isDone());
    delete dfs_it;
}

TEST_F(DfsIteratorTest, EmptyFolderDfsIteratorAllShouldbeCorrect)
{
    Iterator *dfs_it =  new DfsIterator(D3);
    for(dfs_it->first(); !dfs_it->isDone(); dfs_it->next()){
        ASSERT_TRUE(0);
    }
    delete dfs_it;
}